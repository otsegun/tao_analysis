...
*Proof:* Let $n \in \mathbb{N}$. Inducting on $n$ ad fixing $q$, for the base case $n = 0$, we need to show that for some $q\in\mathbb{N+}$ there exist $m$, $r$ $\in \mathbb{N}$ such that $0 = mq+r$. But $0 = 0+0$ and we also know that $0\times q = 0 \forall q\ \in\ \mathbb{N+}$ so $0 = 0\times q + 0$. Taking $m = 0$ and $r = 0$ completes the proof of the base case. 
