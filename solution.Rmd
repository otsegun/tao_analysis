...
**Exercise 2.2.5.** Prove **Proposition 2.2.14**. (Strong principle of induction). Let $m_0$ be a natural number, and Let $P(m)$ be a property pertaining to an arbitrary natural number $m$. Suppose that for each $m \ge m_0$, we have the following implication: if P (m0) is true for all natural numbers $m_0 \le m_0 < m$ , then $P(m)$ is also true. (In particular,
this means that $P(m_0)$ is true, since in this case the hypothesis is vacuous.) Then we can conclude that $P(m)$ is true for all natural numbers $m \ge m0$.

*Solution:* Let $Q(n)$ be the property that $P(m)$ is true for all $m_o \le m < n$. Then if $n\le m_0$, then $Q(n)$ is vacuously true. If $n = m_0{++}$, then $Q(m_0{++})$ is true because $P(m)$ is true for all natural numbers $m_0 \le m < m_0{++}$ and by definition of $P$, $P(m_0{++})$ is also true. Likewise, if $n = (m_0{++})++$, then $Q((m_0{++})++)$ is true becuase  $P(m)$ is true for all $m_0 \le m < (m_0{++})++$ (since $P(m_0)$ and $P(m_0{++})$ are true) and by definition of $P$, $P((m_0{++})++)$ is also true.    
